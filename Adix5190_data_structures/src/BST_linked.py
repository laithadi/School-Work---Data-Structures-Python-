...
class BST:
    ...
    def is_identical(self, other):
        ...
        if node == other._root:
            identical = self._is_identical_aux(self, node._left, other._left, identical)
            identical = self._is_identical_aux(self, node._right, other._right, identical)
        return identical 
                
    def _is_identical_aux(self, node, other, identical):
        
        if node is not None and other is not None:
            if node == other and identical is not False:
                if node._left._value != other._left._value:
                    identical = False 
                identical = self._is_identical_aux(node._left, other._left, identical)
                if node._right._value != other._right._value: 
                    identical = False 
                identical = self._is_identical_aux(node._right, other._right, identical)
    ...
    def is_balanced(self):
        ...
        elif node._left is None and node._right is None and node is not None:
            balanced = True  
    ...
    def is_valid(self):
        ...
        if node is not None:
            if node._left is not None:
                if not node._left._value < node._value:
                    valid = False
            if node._right is not None:
                if not node._right._value > node._value:
                    valid = False
    ...
    def _is_valid_aux(self, node, valid):
        # your code here
        if node is not None:
            if node._left is not None and valid is not False:
                if not node._left._value < node._value:
                    valid = False
                valid = self._is_valid_aux(node._left, valid)
            if node._right is not None and valid is not False:
                if not node._right._value > node._value:
                    valid = False
                valid = self._is_valid_aux(node._right, valid)
        return valid
