...
class List:
    ...
    def insert(self, i, value):
        ...
        if i <= -(self._count):
            ...
            previous = None
            current = self._front
            while x < i and current is not None:
                previous = current
    ...
    def remove(self, key):
        ...
        elif index != -1: #and not len(self) == 1:
            if previous is None:
    ...
    def reverse_r_aux(self, current, temp): 
        if current is not None:
            ...
            if temp._rear is None:
                temp._front = new_node
                temp._rear = new_node
            else:
                temp._rear._next = new_node
                temp._rear = new_node
        return
    ...
    def _swap(self, pln, prn):
        ...
        while current1 != None and current1._value != pln:
            previous1 = current1
            current1 = current1._next
        current2 = self._front
        while current2 != None and current2._value != prn:
            previous2 = current2
            current2 = current2._next
        if current1 != None and current2 != None:
            if previous1 != None:
                previous1._next = current2
            else:
                self._front = current2
            if previous2 != None:
                previous2._next = current1
            else:
                self._front = current1
            temp = current1._next
            current1._next = current2._next
            current2._next = temp
    ...
    def is_identical_r(self, other):
        ...
        if self._count != other._count:
            ...
            self.identical_r_aux(self._front, other._front, identical)

        return identical
    
    def identical_r_aux(self, current, other_node, identical):
        
        if current is not None and current._value == other_node._value:
            identical = self.identical_r_aux(current._next, other_node._next, identical)
        else:
            identical = False
        return
    ...
    def split(self):
        ...
        if len(self) % 2 == 0:
            ...
            for i in range(len(self)//2 + 1, len(self)):
                    target2.append(self[i])
            target1.append(self[len(self)//2])
        while len(self) != 0:
            self.pop(0)
    ...
    def combine(self, source1, source2):
        ...
        if len(source1) == len(source2):
            i = 0
            while i < len(source1):
                new_node = _List_Node(source1[i], None)
                if self._rear is None:
                    self._front = new_node
                    self._rear = new_node
                else:
                    ...
                    self._rear = new_node
                self._count += 1
                new_node = _List_Node(source2[i], None)
                self._rear._next = new_node
                self._rear = new_node
                self._count += 1
                i += 1
